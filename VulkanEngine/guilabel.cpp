...
Label::Label(std::string text, rect2D boundary, std::string fontname, uint32_t fsize) : m_text(text)
/*, m_font(*gEnv->pRessourcesManager->getCFont(fontname, fsize))*/, Widget(text,  boundary)
{
